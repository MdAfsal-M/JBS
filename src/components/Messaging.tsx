...
const Messaging: React.FC<MessagingProps> = ({ isOpen, onClose, selectedJob, isAdmin = false }) => {
  ...
  const getThreads = () => {
    if (isAdmin) {
      return activeTab === "jobs" ? adminJobThreads : adminInternshipThreads;
    }
  ...
  const getFilteredThreads = () => {
    ...
    if (searchQuery) {
      threads = threads.filter(thread => 
        ...
        (isAdmin ? thread.applicant?.name.toLowerCase() : thread.sender?.name.toLowerCase()).includes(searchQuery.toLowerCase())
      );
    }
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() && selectedThread) {
      const message = {
        id: Date.now(),
        sender: isAdmin ? "Admin" : "Alex Johnson",
        message: newMessage,
        timestamp: "Just now",
        isFromEmployer: !isAdmin,
        isFromApplicant: isAdmin
      };
          ...
          <div className="lg:col-span-1 border-r border-border">
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder={isAdmin ? "Search applicants..." : "Search messages..."}
                  value={searchQuery}
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="jobs">
                    {isAdmin ? "Job Applicants" : "Job Applications"}
                  </TabsTrigger>
                  <TabsTrigger value="internships">
                    {isAdmin ? "Internship Applicants" : "Internships"}
                  </TabsTrigger>
                </TabsList>
              </Tabs>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <Avatar className="w-10 h-10">
                        <AvatarImage src={isAdmin ? thread.applicant?.avatar : thread.sender?.avatar} />
                        <AvatarFallback>
                          {isAdmin ? thread.applicant?.name[0] : thread.sender?.name[0]}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mb-1">
                          {isAdmin ? thread.applicant?.name : thread.sender?.name}
                        </p>
              ...
              <>
                ...
                <div className="border-b border-border p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <Avatar className="w-10 h-10">
                        <AvatarImage src={isAdmin ? selectedThread.applicant?.avatar : selectedThread.sender?.avatar} />
                        <AvatarFallback>
                          {isAdmin ? selectedThread.applicant?.name[0] : selectedThread.sender?.name[0]}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {isAdmin ? selectedThread.applicant?.name : selectedThread.sender?.name}
                        </p>
                      </div>
                    </div>
                    <Badge 
                  ...
                  <div className="mt-3 pt-3 border-t border-border">
                    <div className="flex items-center gap-4 text-sm">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{isAdmin ? selectedThread.applicant?.phone : selectedThread.sender?.phone}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Mail className="w-3 h-3 text-muted-foreground" />
                        <span>{isAdmin ? selectedThread.applicant?.email : selectedThread.sender?.email}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 p-4 overflow-y-auto space-y-4">
                  {selectedThread.messages.map((message: any) => (
                    <div
                      key={message.id}
                      className={`flex ${(isAdmin ? message.isFromApplicant : message.isFromEmployer) ? 'justify-start' : 'justify-end'}`}
                    >
                      <div className={`max-w-[70%] p-3 rounded-lg ${
                        (isAdmin ? message.isFromApplicant : message.isFromEmployer)
                          ? 'bg-muted' 
